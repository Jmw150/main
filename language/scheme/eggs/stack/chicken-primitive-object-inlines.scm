...
(define-inline (%wordblock-set! wb i v)
  (if (%immediate? v)
    (%wordblock-set!/immediate wb i v)
...
(define-inline (%append! . lss)
  ...
  (let ((lss
          (let position-at-first-pair ((lss lss))
            (cond
              ((%null? lss)        '() )
...
(define-inline (%list-find pred? ls)
  (let loop ((ls ls))
    (and
      (not (%null? ls))
      (or
        (let ((elm (%car ls))) (and (pred? elm) elm))
        (loop (%cdr ls)) ) ) ) )
