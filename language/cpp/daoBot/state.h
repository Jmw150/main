...
public:
	...
	void setloc(unsigned short x, 
                unsigned short y, 
                unsigned short val) 
      { game_state.layout[y][x] = val; }
	unsigned short getloc(unsigned short x, unsigned short y) 
      { return game_state.layout[y][x]; }
};
...
State::State()
{
	...
	// Place player 1 stones. (generalized board size)
	for (int row = 0; row < XYDIM; row++)
        game_state.layout[row][row] = PLAYER1;
	...
	for (int row = 0; row < XYDIM; row++)
        game_state.layout[row][(XYDIM-1) - row] = PLAYER2;
