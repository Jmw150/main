...
public:
    ...
    // of the range of the array, it prints an error message and exits
    // the program.
    ...
    // changes elements of array to random numbers
    void randomize(int theSize, int limit=100);

    // checks if arrays are the same size and have the same elements
    bool operator==(MyArray &data);

    // checks if arrays are not the same size or have different elements
    bool operator!=(MyArray &data);

    // gives the dot product, arrays do not have to be the same size
