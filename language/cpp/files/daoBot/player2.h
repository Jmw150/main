...
\****************************************************************/
Move Player2::graph_bot_move(unsigned short p, board game_board)
{
    int searchdepth = 0;
...
\****************************************************************/
Move Player2::stat_bot_move(unsigned short p, board game_board)
{
    // ok, planning a move..
    Move move;
...
/**/// where the stats happens 
    ...
    while (!fin.eof() && fin.good()) 
    {
        ...
        if (fileLine.same_state(stateMovetemp) && fileLine.safe(fileLine)) //found move, clean up later
        {
            ...
            cout << "player2 using data! : " << fileLine.str() << endl;
...
\****************************************************************/
Move Player2::rand_bot_move(unsigned short p, board game_board)
{
    return rand_move(p);
}
...
\****************************************************************/
bool Player2::data::same_state(char* b) 
{
    bool isSame = 1;
...
\****************************************************************/
bool Player2::data::same_move(char *b)
{
    bool isSame = 1;
...
\****************************************************************/
bool Player2::data::same_state(const data& b)
{
    bool isSame = 1;
    for (int i = 0 ; i < stateSize ; i++)
...
| Returns: boolean if two moves are the same
\****************************************************************/
bool Player2::data::same_move(const data& b)
{
    bool isSame = 1;
    for (int i = (datSize-moveSize) ; i < datSize ; i++)
        if (stateMove[i] != b.stateMove[i]) 
...
\****************************************************************/
bool Player2::isover(board game_board, int &winner)
{
	unsigned short x, y;
	bool gameover = false;
...
|          possible
\****************************************************************/
list<Move> Player2::safe_space(board game_board, int p, int depth)
{
    // get set of valid moves
    list<Move> moves = move_space(game_board,p);
    cout << "player2 valid moves: " << moves.size() << endl; 
    ...
    cout << "player2 non-suicide moves: " << moves.size() << endl; 
    ...
    cout << "player2 defensible moves: " << moves.size() << endl; 
...
\****************************************************************/
list<Move> Player2::move_space(board game_board, int p) 
{
    list<Move> moves;
    Move testMove;
    int winner = 0;
...
\****************************************************************/
bool Player2::suicide(board game_board, int p) // pruner
{
    int winner;
...
|          player p to lose
\****************************************************************/
bool Player2::defensible(board game_board, int p) // pruner
{
    Move testMove;
    int winner;
    board testBoard;
...
\****************************************************************/
board Player2::makeMove(board game_board, Move move)
{
