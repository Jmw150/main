...
(define (run-turing-machine tm input . rest)
  (if (null? rest)
      (simulate-turing-machine tm input #f -1)
      (let ((x (car rest)))
        (cond ((string? x)
               ;(delete-file x)         ; would not be portable R5RS Scheme
               (call-with-output-file
                x
                (lambda (out) (simulate-turing-machine tm input out -1))))
              ((output-port? x)
               (simulate-turing-machine tm input x -1))
              (else
...
(define (simulate-turing-machine tm input out steps)
  (if (and (list? tm)
           ...
           (or (eqv? out #f)
               (output-port? out))
           (for-all list? tm)
           (for-all (lambda (x) (or (symbol? x) (number? x))) input)
           (for-all positive? (map length tm))
           (for-all symbol? (map car tm))
           (let ((transition-lists (map cdr tm)))
             (and (for-all (lambda (transitions)
                             (and (for-all list? transitions)
                                  (for-all
                                   (lambda (transition)
