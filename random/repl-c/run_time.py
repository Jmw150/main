...
prompt = "c> "
compiler_command = ( "gcc", "-x", "c", "-o", "$outfile", "-",
...
def get_temporary_file_name():
	outfile = tempfile.NamedTemporaryFile(prefix = 'repl-exe')
...
#def print_welcome(): print '''NO WARRANTY. Type ".h" for help. '''.replace( "$version", version.VERSION )
...
def parse_args( argv ):
	parser = OptionParser( version="repl " + version.VERSION )
...
def run( outputfile = sys.stdout, inputfile = None, print_welc = False,
		argv = None ):
	...
	try:
		...
		except dot_commands.REPLQuitException:
			ret = "quit"
	finally:
